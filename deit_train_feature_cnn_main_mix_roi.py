...
def save_checkpoint(epoch, model, optimizer, path=f"model_dumps/{model_name}_train_checkpoint_{slice_count}.pth"):
    torch.save({
        'epoch': epoch,
...
def load_checkpoint(path=f"model_dumps/{model_name}_train_checkpoint_{slice_count}.pth"):
...
checkpoint_path = f"model_dumps/{model_name}_train_checkpoint_{slice_count}.pth"
start_epoch = 0
...
for epoch in range(start_epoch, vit_train_epochs):
    ...
    with open(f'{model_name}_train_metrics_{slice_count}.csv', 'a') as f:
        f.write(f"{epoch+1},{epoch_loss},{epoch_accuracy}\n")
    # Save checkpoint at the end of each epoch
    # shift the model to cpu before saving
    # model.to('cpu')
    save_checkpoint(epoch, model, optimizer, path=f"model_dumps/{model_name}_train_checkpoint_{slice_count}.pth")
...
os.makedirs(f"adni_storage/ADNI_features/{model_name}/train_e{vit_train_epochs}/{slice_count}/", exist_ok=True)
...
for _, row in tqdm(df_adni.iterrows(), total=len(df_adni), desc="Processing images"):
    ...
    feature_file_path = f"adni_storage/ADNI_features/{model_name}/train_e{vit_train_epochs}/{slice_count}/{image_title}_features.npy"
            ...
            try:
                ...
                for slice_idx in range(data.shape[0]):
                    ...
                    with torch.no_grad():
                        ...
                        # slice_features = model.deit(slice_tensor).last_hidden_state.mean(dim=1).squeeze().cpu().numpy()  # Move output back to CPU
                        slice_features = model.deit(slice_tensor).last_hidden_state.mean(dim=1).squeeze().cpu().numpy()
...
# Directory to save processed images and features
# os.makedirs(f"ixi_storage/IXI_features/train_e{vit_train_epochs}/{slice_count}/", exist_ok=True)
# for _, row in tqdm(df_ixi.iterrows(), total=len(df_ixi), desc="Processing test images"):
#     filepath = row['filepath']    
#     image_title = f"{row['ImageID']}"
#         # Check if the feature file already exists
#     feature_file_path = f"ixi_storage/IXI_features/train_e{vit_train_epochs}/{slice_count}/{image_title}_features.npy"
#     if os.path.exists(feature_file_path):
#         # If file exists, load the features from the file
#         features = np.load(feature_file_path)
...
# # Directory to save processed images and features
# os.makedirs(f"abide_storage/ABIDEII_features/train_e{vit_train_epochs}/{slice_count}/", exist_ok=True)
# for _, row in tqdm(df_abide.iterrows(), total=len(df_abide), desc="Processing test images"):
#     filepath = row['filepath']    
#     image_title = f"{row['ImageID'][7:]}"
#         # Check if the feature file already exists
#     feature_file_path = f"abide_storage/ABIDEII_features/train_e{vit_train_epochs}/{slice_count}/{image_title}_features.npy"
#     # feature_file_path = f"ixi_storage/IXI_features/train_e{vit_train_epochs}/{slice_count}/{image_title}_features.npy"
#     if os.path.exists(feature_file_path):
#         # If file exists, load the features from the file
#         features = np.load(feature_file_path)
...
os.makedirs(f"dlbs_storage/DLBS_features/{model_name}/train_e{vit_train_epochs}/{slice_count}/", exist_ok=True)
for _, row in tqdm(df_dlbs.iterrows(), total=len(df_dlbs), desc="Processing test images"):
    filepath = row['filepath']    
    image_title = f"{row['ImageID'][4:]}"
        # Check if the feature file already exists
    feature_file_path = f"dlbs_storage/DLBS_features/{model_name}/train_e{vit_train_epochs}/{slice_count}/{image_title}_features.npy"
    # feature_file_path = f"ixi_storage/IXI_features/train_e{vit_train_epochs}/{slice_count}/{image_title}_features.npy"
    if os.path.exists(feature_file_path):
        # If file exists, load the features from the file
        features = np.load(feature_file_path)
...
# os.makedirs(f"cobre_storage/COBRE_features/train_e{vit_train_epochs}/{slice_count}/", exist_ok=True)
# for _, row in tqdm(df_cobre.iterrows(), total=len(df_cobre), desc="Processing test images"):
#     filepath = row['filepath']    
#     image_title = f"{row['ImageID'][5:]}"
#         # Check if the feature file already exists
#     feature_file_path = f"cobre_storage/COBRE_features/train_e{vit_train_epochs}/{slice_count}/{image_title}_features.npy"
#     # feature_file_path = f"ixi_storage/IXI_features/train_e{vit_train_epochs}/{slice_count}/{image_title}_features.npy"
#     if os.path.exists(feature_file_path):
#         # If file exists, load the features from the file
#         features = np.load(feature_file_path)
...
os.makedirs(f"fcon1000_storage/fcon1000_features/{model_name}/train_e{vit_train_epochs}/{slice_count}/", exist_ok=True)
for _, row in tqdm(df_fcon.iterrows(), total=len(df_fcon), desc="Processing test images"):
    filepath = row['filepath']    
    image_title = f"{row['ImageID'][5:]}"
        # Check if the feature file already exists
    feature_file_path = f"fcon1000_storage/fcon1000_features/{model_name}/train_e{vit_train_epochs}/{slice_count}/{image_title}_features.npy"
    # feature_file_path = f"ixi_storage/IXI_features/train_e{vit_train_epochs}/{slice_count}/{image_title}_features.npy"
    if os.path.exists(feature_file_path):
        # If file exists, load the features from the file
        features = np.load(feature_file_path)
...
os.makedirs(f"sald_storage/SALD_features/{model_name}/train_e{vit_train_epochs}/{slice_count}/", exist_ok=True)
for _, row in tqdm(df_sald.iterrows(), total=len(df_sald), desc="Processing test images"):
    filepath = row['filepath']    
    image_title = f"{row['ImageID'][4:]}"
        # Check if the feature file already exists
    feature_file_path = f"sald_storage/SALD_features/{model_name}/train_e{vit_train_epochs}/{slice_count}/{image_title}_features.npy"
    # feature_file_path = f"ixi_storage/IXI_features/train_e{vit_train_epochs}/{slice_count}/{image_title}_features.npy"
    if os.path.exists(feature_file_path):
        # If file exists, load the features from the file
        features = np.load(feature_file_path)
...
# os.makedirs(f"corr_storage/CORR_features/train_e{vit_train_epochs}/{slice_count}/", exist_ok=True)
# for _, row in tqdm(df_corr.iterrows(), total=len(df_corr), desc="Processing test images"):
#     filepath = row['filepath']    
#     image_title = f"{row['ImageID'][5:]}"
#         # Check if the feature file already exists
#     feature_file_path = f"corr_storage/CORR_features/train_e{vit_train_epochs}/{slice_count}/{image_title}_features.npy"
#     # feature_file_path = f"ixi_storage/IXI_features/train_e{vit_train_epochs}/{slice_count}/{image_title}_features.npy"
#     if os.path.exists(feature_file_path):
#         # If file exists, load the features from the file
#         features = np.load(feature_file_path)
...
os.makedirs(f"oasis1_storage/oasis1_features/{model_name}/train_e{vit_train_epochs}/{slice_count}/", exist_ok=True)
for _, row in tqdm(df_oas1.iterrows(), total=len(df_oas1), desc="Processing test images"):
    filepath = row['filepath']    
    image_title = f"{row['ImageID']}"
        # Check if the feature file already exists
    feature_file_path = f"oasis1_storage/oasis1_features/{model_name}/train_e{vit_train_epochs}/{slice_count}/{image_title}_features.npy"
    if os.path.exists(feature_file_path):
        # If file exists, load the features from the file
        features = np.load(feature_file_path)
...
os.makedirs(f"camcan_storage/CamCAN_features/{model_name}/train_e{vit_train_epochs}/{slice_count}/", exist_ok=True)
for _, row in tqdm(df_camcan.iterrows(), total=len(df_camcan), desc="Processing test images"):
    filepath = row['filepath']    
    image_title = f"{row['ImageID']}"
        # Check if the feature file already exists
    feature_file_path = f"camcan_storage/CamCAN_features/{model_name}/train_e{vit_train_epochs}/{slice_count}/{image_title}_features.npy"
...
#scheduler = optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=10, eta_min=0.0001, last_epoch=-1)
...
if load_saved != "none":
    with open(f"model_dumps/mix/{model_name}/{slice_count}/{sys.argv[1]}_best_model_with_metadata.pkl", "rb") as f:
        checkpoint = pickle.load(f)
    best_loss = checkpoint["loss"]

    # Load the checkpoint
    with open(f"model_dumps/mix/{model_name}/{slice_count}/{sys.argv[1]}_{load_saved}_model_with_metadata.pkl", "rb") as f:
        checkpoint = pickle.load(f)
...
filename = sys.argv[1] 
csv_file = f"model_dumps/mix/{model_name}/{slice_count}/{filename}.csv"
...
def update_loss_plot(epoch_data, filename):
    ...
    df.to_csv(f"model_dumps/mix/{slice_count}/{filename}.csv", index=False)  # Save the data to CSV
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig(f"model_dumps/mix/{slice_count}/{filename}.png")
    plt.close()
...
for epoch in range(start_epoch, epochs):
    ...
    with open(f"model_dumps/mix/{model_name}/{slice_count}/{sys.argv[1]}_last_model_with_metadata.pkl", "wb") as f:
    ...
    if val_loss < best_loss:
        ...
        with open(f"model_dumps/mix/{model_name}/{slice_count}/{sys.argv[1]}_best_model_with_metadata.pkl", "wb") as f:
            pickle.dump(checkpoint, f)
    ...
    # print (train_df)
    train_df.to_csv(f"model_dumps/mix/{model_name}/{slice_count}/{sys.argv[1]}_predicted_ages_train.csv")
    ...
    print (test_df)
    test_df.to_csv(f"model_dumps/mix/{model_name}/{slice_count}/{sys.argv[1]}_predicted_ages_val.csv")
    ...
    plt.grid(True)
    plt.savefig(f"model_dumps/mix/{model_name}/{slice_count}/plots/{model_name}_cnn_{sys.argv[1]}_epoch{epoch}.png")
