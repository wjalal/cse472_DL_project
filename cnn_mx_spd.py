...
class SpatialDropout2d(nn.Dropout2d):
    def forward(self, x):
        ...
        mask = nn.functional.dropout(mask, self.p, self.training, inplace=True)
        return x * mask
...
class AgePredictionCNN(nn.Module):
    def __init__(self, input_shape):
        ...
        self.spatial_dropout1 = SpatialDropout2d(p=0.2)  # Added SpatialDropout2d after conv1

        self.conv2 = nn.Conv2d(1, 1, kernel_size=(5, 15), stride=1)
        self.pool2 = nn.MaxPool2d(kernel_size=2, stride=1)
        self.spatial_dropout2 = SpatialDropout2d(p=0.2)  # Added SpatialDropout2d after conv2

        self.conv3 = nn.Conv2d(1, 1, kernel_size=(2, 6), stride=1)
        self.pool3 = nn.MaxPool2d(kernel_size=2, stride=1)
        self.spatial_dropout3 = SpatialDropout2d(p=0.2)  # Added SpatialDropout2d after conv3
    ...
    def forward(self, x, sex):
        ...
        x = self.swish(x)
        x = self.pool1(x)  # Apply pooling
        x = self.spatial_dropout1(x)  # Apply SpatialDropout2d

        x = self.conv2(x)
        x = self.swish(x)
        x = self.pool2(x)  # Apply pooling
        x = self.spatial_dropout2(x)  # Apply SpatialDropout2d

        x = self.conv3(x)
        x = self.swish(x)
        x = self.pool3(x)  # Apply pooling
        x = self.spatial_dropout3(x)  # Apply SpatialDropout2d
