...
class ADNIDataset(Dataset):
    def __init__(self, features_list, sex_list, labels_list):
...
dataset = ADNIDataset(features_list, sex_encoded, labels_list)
...
for epoch in range(start_epoch, epochs):
    ...
    for idx, (features, sex, labels) in enumerate(train_loader):
        features = features.unsqueeze(1).to(device)  # Add channel dimension
        sex = sex.to(device)
        labels = labels.to(device)

        optimizer.zero_grad()
        outputs = model(features, sex)  # Forward pass
    ...
    with torch.no_grad():
        for idx, (features, sex, labels) in enumerate(val_loader):
            features = features.unsqueeze(1).to(device)  # Add channel dimension
            sex = sex.to(device)
            labels = labels.to(device)
