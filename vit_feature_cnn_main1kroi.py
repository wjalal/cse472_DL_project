...
# Directory to save processed images and features
os.makedirs("adni_storage/ADNI_features/roi", exist_ok=True)
...
for _, row in tqdm(df.iterrows(), total=len(df), desc="Processing images"):
    ...
    feature_file_path = f"adni_storage/ADNI_features/roi/{image_title}_features.npy"
...
if load_saved != "none":
    # Load the checkpoint
    with open(f"model_dumps/{sys.argv[1]}_{load_saved}_model_with_metadata_1kroi.pkl", "rb") as f:
...
filename = sys.argv[1] 
csv_file = f"model_dumps/{filename}1kroi.csv"
...
def update_loss_plot(epoch_data, filename):
    ...
    df.to_csv(f"model_dumps/{filename}1kroi.csv", index=False)  # Save the data to CSV
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig(f"model_dumps/{filename}1kroi.png")
    plt.close()
...
for epoch in range(start_epoch, epochs):
    ...
    if val_loss < best_loss:
        ...
        with open(f"model_dumps/{sys.argv[1]}_best_model_with_metadata1kroi.pkl", "wb") as f:
            pickle.dump(checkpoint, f)
    ...
    # print (train_df)
    # train_df.to_csv(f"model_dumps/{sys.argv[1]}_predicted_ages_train1kroi.csv")
    ...
    # # print (test_df)
    # test_df.to_csv(f"model_dumps/{sys.argv[1]}_predicted_ages_val1kroi.csv")
