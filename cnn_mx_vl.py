...
class AgePredictionCNN(nn.Module):
    def __init__(self, input_shape):
        ...
        self.conv1 = nn.Conv2d(1, 1, kernel_size=(10, 85), stride=1)
        self.pool1 = nn.MaxPool2d(kernel_size=2, stride=1)

        self.conv2 = nn.Conv2d(1, 1, kernel_size=(5, 20), stride=1)
        self.pool2 = nn.MaxPool2d(kernel_size=2, stride=1)

        self.conv3 = nn.Conv2d(1, 1, kernel_size=(2, 10), stride=1)
        ...
        self.fc2_bn = nn.LayerNorm(512)
        self.fc3 = nn.Linear(512, 128)
        self.fc3_bn = nn.LayerNorm(128)
        self.dropout = nn.Dropout(p=0.1)  # Dropout with 10% probability
        self.fc4 = nn.Linear(129, 1)  # Adding 1 for the `Sex` input
    ...
    def initialize_fc1(self, input_shape):
        ...
        self.fc1_bn = nn.LayerNorm(1024)  # Initialize batch normalization for fc1
    ...
    def forward(self, x, sex):
        ...
        x = self.fc3(x)
        x = self.fc3_bn(x)  # Apply batch normalization
        x = self.relu(x)
        x = self.dropout(x)  # Apply dropout
