...
class AgePredictionCNNMulticlass(nn.Module):
    def __init__(self, input_shape, num_classes):
        super(AgePredictionCNNMulticlass, self).__init__()
        ...
        self.fc3 = nn.Linear(130, num_classes)  # Output for multiple classes
    ...
    def forward(self, x, sex, age_gap):
        x = self.conv1(x)
        x = self.lrelu(x)
        x = self.pool1(x)  # Apply pooling
        x = self.conv2(x)
        ...
        # Concatenate `Age Gap` input
        x = torch.cat((x, age_gap.unsqueeze(1)), dim=1)
        x = self.fc3(x)
