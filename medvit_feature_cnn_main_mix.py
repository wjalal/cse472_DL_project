...
os.makedirs("ixi_storage/IXI_features/MedViT", exist_ok=True)
os.makedirs("abide_storage/ABIDEII_features/MedViT", exist_ok=True)
os.makedirs("dlbs_storage/DLBS_features/MedViT", exist_ok=True)
os.makedirs("cobre_storage/COBRE_features/MedViT", exist_ok=True)
os.makedirs("fcon1000_storage/fcon1000_features/MedViT", exist_ok=True)
os.makedirs("sald_storage/SALD_features/MedViT", exist_ok=True)
...
for _, row in tqdm(df_ixi.iterrows(), total=len(df_ixi), desc="Processing test images"):
    ...
    image_title = f"{row['ImageID']}"
        # Check if the feature file already exists
    feature_file_path = f"ixi_storage/IXI_features/MedViT/{image_title}_features.npy"
    if os.path.exists(feature_file_path):
        # If file exists, load the features from the file
        features = np.load(feature_file_path)
        ...
        if os.path.exists(filepath):
            try:
                ...
                for slice_idx in range(data.shape[0]):
                    ...
                    with torch.no_grad():
                        outputs = model(slice_tensor, return_embeddings=True)
                        slice_features = outputs.squeeze().cpu().numpy()
                        features.append(slice_features)
                # Save extracted features
                features = np.array(features)
                np.save(feature_file_path, features)
...
for _, row in tqdm(df_abide.iterrows(), total=len(df_abide), desc="Processing test images"):
    ...
    image_title = f"{row['ImageID'][7:]}"
        # Check if the feature file already exists
    feature_file_path = f"abide_storage/ABIDEII_features/MedViT/{image_title}_features.npy"
...
for _, row in tqdm(df_dlbs.iterrows(), total=len(df_dlbs), desc="Processing test images"):
    ...
    image_title = f"{row['ImageID'][4:]}"
        # Check if the feature file already exists
    feature_file_path = f"dlbs_storage/DLBS_features/MedViT/{image_title}_features.npy"
    # feature_file_path = f"ixi_storage/IXI_features/{image_title}_features.npy"
    if os.path.exists(feature_file_path):
        # If file exists, load the features from the file
        features = np.load(feature_file_path)
...
for _, row in tqdm(df_cobre.iterrows(), total=len(df_cobre), desc="Processing test images"):
    ...
    image_title = f"{row['ImageID'][5:]}"
        # Check if the feature file already exists
    feature_file_path = f"cobre_storage/COBRE_features/MedViT/{image_title}_features.npy"
    # feature_file_path = f"ixi_storage/IXI_features/{image_title}_features.npy"
    if os.path.exists(feature_file_path):
        # If file exists, load the features from the file
        features = np.load(feature_file_path)
...
for _, row in tqdm(df_fcon.iterrows(), total=len(df_fcon), desc="Processing test images"):
    ...
    image_title = f"{row['ImageID'][5:]}"
        # Check if the feature file already exists
    feature_file_path = f"fcon1000_storage/fcon1000_features/MedViT/{image_title}_features.npy"
    # feature_file_path = f"ixi_storage/IXI_features/{image_title}_features.npy"
    if os.path.exists(feature_file_path):
        # If file exists, load the features from the file
        features = np.load(feature_file_path)
...
for _, row in tqdm(df_sald.iterrows(), total=len(df_sald), desc="Processing test images"):
    ...
    image_title = f"{row['ImageID'][4:]}"
        # Check if the feature file already exists
    feature_file_path = f"sald_storage/SALD_features/MedViT/{image_title}_features.npy"
    # feature_file_path = f"ixi_storage/IXI_features/{image_title}_features.npy"
    if os.path.exists(feature_file_path):
        # If file exists, load the features from the file
        features = np.load(feature_file_path)
...
for epoch in range(start_epoch, epochs):
    ...
    plt.grid(True)
    plt.savefig(f"model_dumps/MedViT/plots/vit_cnn_{sys.argv[1]}_epoch{epoch}.png")
